...
def preprocess_cgga_data():
    print("🚀 Starting data preprocessing for CGGA-693...")
    
    raw_dir = "data/raw/cgga"
    processed_dir = "data/processed/test_cgga"
    ...
    # Read only the header to get genes
    ...
    # 2. Load CGGA Expression Data
    print("Loading CGGA mRNA-seq 693 expression data...")
    expr_path = os.path.join(raw_dir, "CGGA.mRNAseq_693.RSEM-genes.20200506.txt")
    ...
    # 3. Load CGGA Clinical Data
    print("Loading CGGA clinical data...")
    clinical_path = os.path.join(raw_dir, "CGGA.mRNAseq_693_clinical.20200506.txt")
    clinical_df = pd.read_csv(clinical_path, sep='\t')
    
    # Required columns and renaming to match TCGA format used in evaluation
    # CGGA columns: 'CGGA_ID', 'OS', 'Censor (alive=0; dead=1)'
    ...
    print(f"✅ Processed CGGA data saved to {processed_dir}")

if __name__ == "__main__":
    preprocess_cgga_data()
