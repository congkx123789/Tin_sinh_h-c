...
Mỗi mô hình được khởi tạo với một hạt giống ngẫu nhiên (`seed`) khác nhau chạy từ `42` đến `51`.
- Mỗi mô hình con được huấn luyện trên một tập mẫu con được phân tách khác nhau.
- **Giá trị rủi ro cuối cùng (Final Prognostic Risk Score)** của bệnh nhân $i$ là giá trị trung bình cộng (average aggregation) từ dự đoán của 10 mô hình con:
  $$\widehat{y}_i = \frac{1}{M} \sum_{m=1}^{M} f_m(x_i)$$
  Trong đó $M = 10$ là số lượng mô hình con trong Ensemble, và $f_m(x_i)$ là đầu ra dự đoán thời gian sống (hoặc rủi ro) của mô hình thứ $m$.
...
#### Trích đoạn mã nguồn thực thi chính của Hà Vũ Công (`scripts/train_ultimate.py`):

```python
# Khởi tạo danh sách lưu trữ các mô hình con trong Ensemble
models = []
num_models = 10

# Nhãn mục tiêu được biến đổi log để ổn định phương sai
...
for i in range(num_models):
    params = {
        ...
        'bagging_freq': 1,
        'lambda_l1': 1.0,  # Điều chuẩn L1
        'lambda_l2': 1.0,  # Điều chuẩn L2
        'seed': 42 + i     # Thay đổi hạt giống ngẫu nhiên
    }
...
- **Cộng nhiễu đầu vào (Denoising Process)**: Trong quá trình huấn luyện, biểu hiện gen $x$ sẽ bị cộng thêm một lượng nhiễu Gauss ngẫu nhiên $\epsilon \sim \mathcal{N}(0, \sigma^2)$ với `noise_factor = 0.2` tạo thành $x_{\text{noisy}} = x + 0.2 \cdot \epsilon$. Phép toán này ép mạng mạng học sâu không được ghi nhớ máy móc (memorize) dữ liệu đầu vào mà phải học cách lọc bỏ nhiễu để tái cấu trúc.
- **Cơ chế nén**: 
  - Lớp mã hóa (Encoder) gồm chuỗi các lớp tuyến tính nén dần số chiều từ đầu vào ($16.000+$) xuống $512$, $256$ và cuối cùng hội tụ tại không gian ẩn (latent space) **$128$ chiều** ($z \in \mathbb{R}^{128}$).
  - Hàm kích hoạt phi tuyến `ReLU` được áp dụng ở các lớp ẩn để mô phỏng sự tương tác không tuyến tính giữa các gen.
...
*Tại sao sử dụng Mamba trong tin sinh học?* Kiến trúc Transformer truyền thống đòi hỏi chi phí tính toán và bộ nhớ bình phương $O(L^2)$ theo chiều dài chuỗi đặc trưng do cơ chế Attention. Mamba kế thừa ưu điểm của mô hình không gian trạng thái (State Space Models - SSMs), giải quyết bài toán với độ phức tạp tuyến tính $O(L)$ đồng thời sở hữu khả năng chọn lọc thông tin (content-aware selection mechanism) tối ưu bằng cách cho phép các ma trận chuyển trạng thái phụ thuộc động vào dữ liệu đầu vào.

Trong SurvivalMambaNet, khối Mamba coi $128$ chiều ẩn giống như một chuỗi các đặc trưng sinh học biểu diễn có tính phân bậc. Mô hình thực hiện phép rời rạc hóa hệ thống động liên tục:
$$h_t = A h_{t-1} + B z_t$$
$$y_t = C h_t + D z_t$$
Trong đó các ma trận $B, C$ được suy biến động trực tiếp từ $z_t$ thông qua các lớp tuyến tính tích hợp bên trong lõi Mamba. Khối này giúp mô hình hóa sự phụ thuộc phức tạp, bắc cầu giữa các nhóm chức năng gen trong không gian ẩn mà các mạng nơ-ron truyền thống (MLP) thường bỏ sót.

Đầu ra của khối Mamba được đưa qua một lớp chiếu tuyến tính (Projection Layer) để cho ra một giá trị thực duy nhất: **Chỉ số rủi ro rủi ro nguy cơ (Prognostic Risk Score - $h_i$)** đại diện cho khả năng tử vong của bệnh nhân $i$.
...
Hàm số này cho phép mạng nơ-ron xử lý trực tiếp các dữ liệu bị khuyết góc (censored data - bệnh nhân vẫn còn sống tại thời điểm kết thúc nghiên cứu):
$$\mathcal{L}_{\text{Cox}}(\theta) = - \sum_{i: \delta_i = 1} \left( h_i(\theta) - \log \sum_{j \in \mathcal{R}(t_i)} \exp(h_j(\theta)) \right)$$
Trong đó:
- $\delta_i \in \{0, 1\}$ là chỉ số sự kiện của bệnh nhân $i$ ($\delta_i = 1$ biểu thị bệnh nhân đã tử vong, $\delta_i = 0$ biểu thị bệnh nhân bị khuyết góc/còn sống).
- $t_i$ là thời gian sống sót của bệnh nhân $i$.
- $\mathcal{R}(t_i) = \{j : t_j \ge t_i\}$ là **Tập nguy cơ (Risk set)** tại thời điểm $t_i$, bao gồm tất cả các bệnh nhân có thời gian sống kéo dài ít nhất bằng thời gian sống của bệnh nhân $i$.
- $h_i(\theta)$ là chỉ số rủi ro do mô hình mạng học sâu SurvivalMambaNet dự đoán cho bệnh nhân $i$ với bộ trọng số mạng $\theta$.

Cơ chế lan truyền ngược (backpropagation) tính toán đạo hàm của $\mathcal{L}_{\text{Cox}}$ theo tham số $\theta$ của Mamba và DAE, từ đó cập nhật toàn bộ trọng số mạng nơ-ron đồng thời để học cách phân tách tốt nhất đường cong Kaplan-Meier của các nhóm bệnh nhân nguy cơ cao và nguy cơ thấp.
