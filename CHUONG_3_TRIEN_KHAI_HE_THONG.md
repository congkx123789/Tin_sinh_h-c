...
2. **Khởi tạo môi trường chuyên biệt (`gbm_mamba`)** với Python 3.10.
3. **Cài đặt các gói thư viện cốt lõi tối ưu**:
   Để tránh xung đột giữa thư viện thống kê sinh học `lifelines`, thư viện giải thích mô hình `shap`, và framework học máy `lightgbm`, quá trình cài đặt được thực thi đồng thời thông qua kênh `conda-forge`.
4. **Xuất cấu hình môi trường để tái lập (Reproducibility)** phục vụ cho việc triển khai trên các máy chủ khác.
...
Quá trình hoạt động tuân theo giải thuật tối ưu:
...
- Tìm kiếm 5 mẫu bệnh nhân có cấu hình biểu hiện gen tương đồng nhất với bệnh nhân bị thiếu dữ liệu dựa trên việc tính toán khoảng cách hình học (Euclid) trên không gian đặc trưng đa chiều.
- Điền giá trị khuyết bằng giá trị trung bình có trọng số của các láng giềng gần nhất này. Quy trình này giúp bảo toàn mối tương quan sinh học phức tạp giữa các cụm gen tương tác mà không làm mất đi tính tự nhiên của dữ liệu.
...
*Nguyên lý hoạt động*: Thuật toán xây dựng một rừng các cây quyết định phân tách ngẫu nhiên. Các điểm dị biệt (outliers) nằm tách biệt trong không gian đa chiều sẽ dễ dàng bị cô lập hơn nhiều so với các điểm bình thường. Do đó, các mẫu dị biệt sẽ có độ dài đường đi trung bình từ gốc đến lá cực kỳ ngắn trên cây quyết định. Những bệnh nhân được phân loại là mẫu dị biệt sẽ bị loại bỏ khỏi toàn bộ tập huấn luyện để tránh mô hình bị lệch hướng học (bias).

#### 2. Biến đổi Log1p
Biểu hiện gen thường tuân theo phân phối lệch phải cực kỳ nặng với một vài gen có mức biểu hiện siêu cao. Hệ thống thực hiện phép biến đổi phi tuyến bằng cách lấy logarit tự nhiên của giá trị biểu hiện gen cộng thêm một đơn vị. Phép toán này giúp làm co hẹp dải giá trị cực đại, đưa phân phối biểu hiện gen về dạng gần phân phối chuẩn, giảm thiểu hiện tượng sai số thay đổi theo giá trị lớn và giúp các thuật toán học máy hội tụ nhanh hơn.

#### 3. Robust Scaler (Chuẩn hóa chống dị biệt đặc trưng)
Thay vì sử dụng StandardScaler (vốn rất nhạy cảm với các đột biến biểu hiện cực đoan của tế bào ung thư), hệ thống sử dụng **Robust Scaler**. Phương pháp này chuẩn hóa dữ liệu dựa trên giá trị trung vị (Median) và khoảng biến thiên tứ phân vị (IQR).

> [!TIP]
> Sử dụng Robust Scaler giúp hệ thống duy trì được tính ổn định cao. Các giá trị biểu hiện gen đột biến cực đoan mang giá trị chẩn đoán lâm sàng quan trọng không bị triệt tiêu, đồng thời đảm bảo thang đo của các đặc trưng đồng đều nhau trong suốt quá trình học máy.
...
2. **Hợp nhất dữ liệu (Feature Joining)**:
   Hệ thống thực hiện phép giao (intersection) trên tập mẫu bệnh nhân giữa ma trận biểu hiện gen X và nhãn sống sót y chứa thêm các đặc trưng đột biến từ GATK tạo ra tệp tích hợp. Sau đó, trong quá trình huấn luyện nâng cao, các đặc trưng này sẽ được kết hợp đồng thời cùng ma trận biểu hiện gen X để tối ưu hóa khả năng dự đoán phân tầng của mô hình.
...
Hệ thống sử dụng mô hình hồi quy phân tích sống sót cổ điển **CoxPH** với kỹ thuật L2 Regularization để kiểm soát nhiễu đa cộng tuyến giữa các gen biểu hiện cùng hướng. Mô hình được dùng để tính toán mức độ ảnh hưởng của biểu hiện từng gen lên thời gian sống sót của bệnh nhân, từ đó làm cơ sở chọn lọc đặc trưng sinh học.

Mô hình CoxPH được khớp trên dữ liệu của 50 gen ứng viên để đánh giá mức độ rủi ro tử vong độc lập.

#### Bước 4: Trích xuất đặc trưng có ý nghĩa thống kê (p-value < 0.05)
Từ bảng tổng hợp kết quả huấn luyện của CoxPH, hệ thống thực hiện kiểm định ý nghĩa thống kê Wald để xác định tầm ảnh hưởng của các gen. Chỉ giữ lại những gen có độ tin cậy thực nghiệm cao:
- Chỉ số p-value < 0.05.
...
> [!IMPORTANT]
> Phương pháp kết hợp đa tầng này rút gọn tập gen từ hơn 16.000 xuống còn 23-30 gen biomarkers tối ưu nhất (ví dụ: *HNRNPA1P52, ANKRD1, SCG5, RFX8, PARK7*). Sự tinh giản này giúp nâng cao độ ổn định và khả năng tổng quát hóa của mô hình trên các tập dữ liệu ngoại kiểm độc lập, đồng thời hỗ trợ các nhà nghiên cứu sinh tin học dễ dàng tập trung phân tích chức năng sinh học của các biomarkers chính yếu.

---

## Tóm tắt chương

Hệ thống đã được đóng gói hoàn chỉnh và tích hợp thành công các cấu phần của đường ống dữ liệu. Việc quản lý môi trường bằng **Mamba** đảm bảo tính nhất quán của các thư viện, quy trình tiền xử lý giúp loại bỏ nhiễu bệnh nhân và chuẩn hóa dữ liệu biểu hiện gen. Bộ lọc đặc trưng đa tầng kết hợp tương quan và hồi quy CoxPH đã trích xuất thành công các dấu ấn di truyền quan trọng để làm đầu vào cho mô hình **Ensemble LightGBM**. Hệ thống được hoàn thiện dưới dạng một ứng dụng **Streamlit Dashboard** trực quan, sẵn sàng phục vụ việc chẩn đoán tiên lượng và giải thích y khoa.
