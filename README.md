...
1.  **Kích hoạt nhân GPU tối ưu**: Chuyển đổi từ chế độ "Giả lập Linear" sang **Real Mamba Kernels** (Selective Scan). Sử dụng các thư viện `causal-conv1d` và `mamba-ssm` được tối ưu hóa riêng cho kiến trúc CUDA.
2.  **Môi trường hiện đại**: Thiết lập hệ thống chạy trên **PyTorch** phiên bản mới nhất và tương thích hoàn toàn với **CUDA**.
...
- **Bước 7: Nâng cấp GPU**: Xây dựng môi trường ảo `mamba_env` với các gói Wheel chuyên biệt để chạy mô hình trên phần cứng hỗ trợ CUDA.
...
Hệ thống hiện tại đạt được hiệu suất cực kỳ ổn định trên nhiều nền tảng:

### 1. Kết quả trên tập dữ liệu chính:
...
### 2. Kết quả kiểm chứng ngoại kiểm đa nguồn (Multi-source Validation):
Chúng tôi đã mở rộng quy mô kiểm chứng lên hàng ngàn mẫu bệnh nhân từ nhiều quốc gia và nền tảng công nghệ khác nhau:

| Bộ dữ liệu | Nền tảng | Số mẫu | Trạng thái | C-index |
| :--- | :--- | :--- | :--- | :--- |
| **CGGA-693** | RNA-Seq | 693 | Hoàn thành | **0.5793** |
| **CGGA-325** | RNA-Seq | 325 | Hoàn thành | **0.6325** |
| **GSE4271** | Microarray | 77 | Hoàn thành | **0.5338** |
| **GSE7696** | Microarray | 80 | Hoàn thành | **0.5523** |
| **REMBRANDT** | Microarray | 490 | Đang xử lý | *Đang cập nhật* |
| **GSE13041** | Microarray | 191 | Đang xử lý | *Đang cập nhật* |
| **GSE4412** | Microarray | 85 | Đang xử lý | *Đang cập nhật* |

---

## 🛠 Những đóng góp mới (Recent Updates)

1.  **Pipeline đồng bộ hóa Gen tự động**: Phát triển script `scripts/preprocess_geo.py` và các bản tùy chỉnh để tự động tham chiếu hàng vạn probe Affymetrix sang Gene Symbol chuẩn TCGA thông qua REST API.
2.  **Tự động nhận diện quy mô biểu hiện (Log-scale Detection)**: Mô hình hiện có khả năng tự động nhận diện dữ liệu đã được log-transform hay chưa để tránh sai số trong tiền xử lý.
3.  **Xử lý đơn vị thời gian survival**: Tích hợp khả năng quy đổi tự động từ Ngày/Tuần/Tháng về một đơn vị chuẩn để đánh giá mô hình.
